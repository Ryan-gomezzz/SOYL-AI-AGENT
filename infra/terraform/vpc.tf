...
resource "aws_nat_gateway" "main" {
  ...
  tags = merge(
    local.common_tags,
    {
      Name = "${local.project_prefix}-nat-${count.index + 1}"
    }
  )
...
# Bastion Security Group (NOTE: Currently allows SSH from anywhere - MUST be locked down in production)
resource "aws_security_group" "bastion" {
  ...
  ingress {
    description = "SSH from anywhere (TEMPORARY - lock down in production)"
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
...
# ALB Security Group (placeholder for future use)
...
resource "aws_security_group" "db" {
  name        = "${local.project_prefix}-db-sg"
